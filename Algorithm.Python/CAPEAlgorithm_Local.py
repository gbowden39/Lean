...
class CAPEAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self._symbols = []
        self._currCape = None
        self._SPX_close = None
        # add SPX data
        self.AddData(SPX, "SPX", Resolution.Daily)
    ...
    def OnData(self, data):
        ...
        if (self._currCape is not None) and (self._SPX_close is not None):   
            try:
                ...
                if self.rebal_flag:
                    ...
                    for stock in self._symbols:
                        ...
                        self.Debug("Rebalancing: " + str(stock)
                            + "   Price: " + str(round(self.Securities[stock].Price, 2)) + "   Quantity: " + str(s.Quantity))
        ...
        if not (data.ContainsKey("CAPE") and data.ContainsKey("SPX")): return
        self._currCape = data["CAPE"].Cape
        self._SPX_close = data["SPX"].Close
        self.Debug("Current Close: " + str(self._SPX_close) + " on " + str(self.Time))
...
class Cape(PythonData):
    ...
    def GetSource(self, config, date, isLiveMode):
        ...
        # return SubscriptionDataSource("https://www.dropbox.com/s/td8arxal2epmjyq/cape_data.csv?dl=1", SubscriptionTransportMedium.RemoteFile)
...
class SPX(PythonData):
    '''SPX Custom Data Class'''
    def GetSource(self, config, date, isLiveMode):
        return SubscriptionDataSource("https://www.dropbox.com/s/aeltztrzq53gn9j/SPX.csv?dl=1", SubscriptionTransportMedium.RemoteFile)
